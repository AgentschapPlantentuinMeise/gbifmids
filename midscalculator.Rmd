...
Set the value for the property of a record to `known:withheld` if its Darwin Core property name is present in the `dwc:informationWithheld` field.

This is currently done for the following Darwin Core properties, using a custom function:

```{r infowithheld}
#better set as a loop and list property names in a vector
occ = infwith(occ,"locality")
occ = infwith(occ,"decimalLatitude")
occ = infwith(occ,"decimalLongitude")
occ = infwith(occ,"county")
occ = infwith(occ,"recordedBy")
occ = infwith(occ,"recordedByID")
occ = infwith(occ,"eventDate")
occ = infwith(occ,"year")
occ = infwith(occ,"verbatimEventDate")
```
...
* No value for `dwc:county`, `dwc:locality` and decimal coordinates.
* No value or any alphanumeric character for `dwc:recordedBy`. No value for `gbif:recordedByID`.
* No value for `dwc:eventDate`, `dwc:year` and `dwc:verbatimEventDate`.

```{r one}
occ$localitylow = tolower(
  gsub("[[:punct:]]|\\s",
       "",
       occ$locality)
  )

occ$countylow = tolower(
  gsub("[[:punct:]]|\\s",
       "",
       occ$county)
)

occ$recordedBylow = tolower(
  gsub("[[:punct:]]|\\s",
       "",
       occ$recordedBy)
)
